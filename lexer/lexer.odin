...
next_token :: proc(l: ^Lexer) -> Token {
	...
	case byte(Symbols.PERCENT):     return make_token(l, match(l, byte(Symbols.EQ)) ? .PERCENT_EQ : .PERCENT)
	...
	case byte(Symbols.STAR):        return make_token(l, match(l, byte(Symbols.EQ)) ? .STAR_EQ    : .STAR)
