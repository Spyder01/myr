...
Local :: struct {
    name:                  string,
    depth:                 u8,
    slots:                 int,              // 1 for scalars/pointers, N for flat structs/enums, N*elem_slots for arrays, 4 for slices
    ...
    array_elem_type_idx:   parser.TypeIdx,   // TypeIdx of the array's element type; parser.INVALID_IDX if not an array
    slice_elem_slots:      int,              // 0 = not a slice; >0 = slots per element
    slice_elem_type_idx:   parser.TypeIdx,   // TypeIdx of the slice's element type; parser.INVALID_IDX if not a slice
}
...
add_local :: proc(compiler: ^ByteCodeCompiler, name: string, slots: int = 1, struct_type: string = "", ptr_inner: string = "", enum_type: string = "", array_elem_slots: int = 0, slice_elem_slots: int = 0, array_elem_type_idx: parser.TypeIdx = parser.TypeIdx(parser.INVALID_IDX), slice_elem_type_idx: parser.TypeIdx = parser.TypeIdx(parser.INVALID_IDX)) -> Maybe(ByteCodeCompilerError) {
    if len(compiler.locals) >= MAX_LOCAL_VARIABLE_COUNT do return .TOO_MANY_LOCAL_VARIABLES
    append(&compiler.locals, Local{name = name, depth = compiler.scope_depth, slots = slots, struct_type = struct_type, ptr_inner = ptr_inner, enum_type = enum_type, array_elem_slots = array_elem_slots, array_elem_type_idx = array_elem_type_idx, slice_elem_slots = slice_elem_slots, slice_elem_type_idx = slice_elem_type_idx})
    return nil
}
